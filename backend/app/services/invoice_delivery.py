...
def send_invoice_email(invoice, recipient_email: str) -> str:
    ...
    payload = {
        ...
        "invoice": {
            ...
            "amount_paid": str(invoice.amount_paid) if getattr(invoice, 'amount_paid', None) is not None else "0.00",
            "status": invoice.status,
            "payment_method": invoice.payment_method,
            "cgst": str(invoice.cgst) if getattr(invoice, 'cgst', None) is not None else "0.00",
            "sgst": str(invoice.sgst) if getattr(invoice, 'sgst', None) is not None else "0.00",
            "igst": str(invoice.igst) if getattr(invoice, 'igst', None) is not None else "0.00",
            "expected_payment_date": invoice.expected_payment_date.isoformat() if getattr(invoice, 'expected_payment_date', None) is not None else None,
        },
